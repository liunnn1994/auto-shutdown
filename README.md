...
```bash
cargo build --release
# 产物: target/release/auto-shutdown.exe，建议放入开机自启
```
...
| 取消关机重弹 | 60 秒 | `src/protocol.rs::SNOOZE_CANCEL_SECONDS` |

## 配置

保存服务地址后写入 `%APPDATA%\auto-shutdown\config.json`：

```json
{ "server": "192.168.1.100:8123" }
```

地址支持三种写法：`192.168.1.100`、`192.168.1.100:8123`、`ws://192.168.1.100:8123`。
...
- **PC 睡眠时会误判吗？** 心跳线程随进程暂停，唤醒后第一次心跳成功即恢复
  正常状态；不会在睡眠中触发关机。
- **扫描不到设备？** 先确认服务端与 PC 在同一网段且无 AP 隔离；也可手动输入
  地址后用"测试"按钮验证。Windows 防火墙一般不影响本应用的出站流量。
- **如何彻底退出？** 右键托盘图标 → 退出程序。
