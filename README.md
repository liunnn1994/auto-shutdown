...
本软件**不持久化任何配置数据**：不写配置文件，每次启动都会重新扫描局域网。
地址支持三种写法：`192.168.1.100`、`192.168.1.100:8123`、
`ws://192.168.1.100:8123`（仅在本次运行内生效）。旧版本遗留的
`%APPDATA%\auto-shutdown\config.json` 会在启动时自动清理。

### 日志

运行日志写入 `~/.auto-shutdown/logs/`（按天分割，文件名 = 类型 + 日期，
自动删除 30 天前的日志），两类日志分开存放，排查"误弹倒计时"时可逐条对照：

| 文件 | 内容 |
|---|---|
| `app-YYYY-MM-DD.log` | PC 端主程序日志：每次心跳成败（rtt / 完整错误链）、扫描、测试、倒计时弹出与用户选择、关机命令 |
| `esp-YYYY-MM-DD.log` | ESP8266 随心跳连接上报的运行日志：启动（含复位原因）、WiFi 掉线/重连、配置热点开关、配置变更、非法报文、5 分钟一条的存活统计 |

> ESP 端自身不写文件：日志先存放在设备内存的环形缓冲（最近 24 条），
> 每次心跳连接建立时随 `pong` 之前推送给 PC 落盘。PC 端记录的是收到时刻，
> 条目中的 `uptime=Xs` 是设备开机秒数——若发现 uptime 归零，说明设备发生过重启。
...
├── logger.rs     文件日志（~/.auto-shutdown/logs，按天分割，保留 30 天）
