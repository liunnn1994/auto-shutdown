...
- **日志上报**：启动（含复位原因）、WiFi 掉线/重连、配置热点开关、配置变更、
  非法报文（限流）、5 分钟一条的存活统计等事件记入内存环形缓冲（最近 24 条），
  随下一次心跳连接推送给 PC，由 PC 端写入 `~/.auto-shutdown/logs/esp-日期.log`
  （与 PC 自身日志 `app-日期.log` 分开存放，均保留 30 天）。
